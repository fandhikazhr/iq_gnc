...
class DroneYoloController:
    def __init__(self):
        ...
        # Load YOLOv5 model
        model_path = "/yolov5_model_trained_path/best.pt"
        ...
        self.detection_times = []  # Store detection times
    ...
    def mission_summary(self):
        if self.mission_start_time:
            ...
            mission_duration = self.mission_end_time - self.mission_start_time
            avg_detection_time = np.mean(self.detection_times)
            rospy.loginfo(f"Mission completed in {mission_duration:.2f} seconds")
            rospy.loginfo(f"Average Detection Time: {avg_detection_time:.4f} seconds per frame")
            
            # Plotting the detection times
            plt.plot(self.detection_times)
            plt.xlabel("Detection Frame")
            plt.ylabel("Detection Time (seconds)")
            plt.title("YOLOv5 Detection Time per Frame")
             # Menyimpan gambar plot
            plt.savefig("where_path_savePlot/yolo_detection_time_plot.png")  # Simpan sebagai PNG
            rospy.loginfo("Plot saved as yolo_detection_time_plot.png")
            plt.show()
...
if __name__ == '__main__':
    try:
        ...
        # After mission is done, print summary
